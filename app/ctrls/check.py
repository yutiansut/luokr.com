...
from lib.captcha.image import gen_captcha, gen_randoms
...
class CheckCtrl(BasicCtrl):
    def get(self, _ext='.jpeg'):
        ...
        self.set_secure_cookie('_code', self.get_escaper().json_encode(
            dict(code=self.utils().str_md5_hex(self.utils().str_md5_hex(self.settings['cookie_secret']) + text.lower() + str(time)), time=time)), expires_days=None)
